...
This guide covers the 14-step worktree-based workflow for **.NET/ASP.NET Core backend development**. This workflow integrates:
...
> *(Docker projects only)* The script automatically starts Docker containers with unique ports, providing a completely isolated .NET environment with a separate database (SQL Server/PostgreSQL) per worktree — no shared resources.
