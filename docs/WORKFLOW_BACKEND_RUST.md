...
This guide covers the 13-step worktree-based workflow for **Rust backend development**. This workflow integrates:
...
> *(Docker projects only)* The script automatically starts Docker containers with unique ports, providing a completely isolated Rust environment with a separate database per worktree.
